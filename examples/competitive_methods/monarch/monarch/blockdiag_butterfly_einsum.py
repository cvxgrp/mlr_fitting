...
from monarch.low_rank import low_rank_project

"""
Code from https://github.com/HazyResearch/fly
"""
...
def blockdiag_butterfly_project_einsum_rank(M, nblocks1, nblocks2, rank):
    ...
    Arguments:
        M: (m, n)
    Outputs:
        w1_bfly: (nblocks1, r * nblocks2, i), where n = i * nblocks1
        w2_bfly: (nblocks2, l, nblocks1 * r), where m = l * nblocks2
    """
    m, n = M.shape
    k, j = nblocks1, nblocks2
    M_permuted_batched = rearrange(M, '(l j) (k i) -> k j l i', k=nblocks1, j=nblocks2)
